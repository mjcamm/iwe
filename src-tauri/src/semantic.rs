...
fn resolve_model_dir(app_handle: &tauri::AppHandle) -> Result<std::path::PathBuf, String> {
    // In dev mode, resource_dir points to target/debug/ which doesn't have our files.
    // Try the Tauri resource resolver first, then fall back to the source resources dir.
    ...
    // Fallback for dev: relative to the Cargo manifest dir
...
fn generate_embedding(
    ...
    // Try 2 inputs first (mpnet), fall back to 3 (miniLM) if needed
    ...
    for t in 0..seq_len {
        ...
        for h in 0..hidden_size {
            // Index into flat [1, seq_len, hidden_size] array
...
fn index_chapter(
    ...
    // Throttle: small pause between embeddings to avoid hogging CPU
...
pub fn run_semantic_indexing(
    ...
    // Spawn background thread so we don't block the IPC thread
...
pub fn rebuild_semantic_index(
    ...
    // Force clear the active flag so it definitely runs
    sem_state.indexing_active.store(false, Ordering::SeqCst);
